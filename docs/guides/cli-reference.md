...
| `--features`   | string    | Comma-separated subset of `convex`, `better-auth`.             |
...
config-level refinement enforces that `better-auth` requires `convex`.
...
turbocraft create my-app \
  ...
  --features convex,better-auth --pm pnpm
```
...
| *n*  | `turbocraft add` passes through the wrapped process's code. |
